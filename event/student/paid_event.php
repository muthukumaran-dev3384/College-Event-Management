...
<?php
$sql = "
    SELECT id, event_name, event_date, venue, fee, rules_file, status
    FROM events
    WHERE department = ?
      AND fee > 0
    ORDER BY event_date
...
if (!$stmt) {
    die("Paid Event SQL Error: " . $conn->error);
}
...
if ($res->num_rows > 0) {
    ...
    while ($e = $res->fetch_assoc()) {
        ...
        if ($e['status'] === 'open') {
            ...
            $action = "<a class='btn pay' href='register_event.php?event_id={$e['id']}'> Register</a>";
        } else {
    ...
    echo "<tr>
            <td colspan='7' class='empty'>No paid events available for your department</td>
          </tr>";
}
?>
